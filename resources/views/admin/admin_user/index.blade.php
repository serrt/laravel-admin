            ...
            <form action="" class="form-horizontal" autocomplete="off">
                ...
                <div class="form-group">
                    <div class="col-md-4 pull-right">
                        ...
                        <a href="{{route('admin.admin_user.create')}}" class="btn btn-default"><i class="fa fa-plus"></i> 添加</a>
                    </div>
                </div>
            </form>
        </div>
            ...
            <table class="table table-bordered table-hover">
                ...
                @foreach($list as $item)
                    <tr>
                        <td>{{$item->id}}</td>
                        <td>
                            <a href="{{route('admin.admin_user.edit', $item)}}" class="btn-link">{{$item->username}}</a>
                        </td>
                        <td>{{$item->name}}</td>
                        <td>
                        ...
                        <td>
                            <a href="{{route('admin.admin_user.show', $item)}}" class="btn btn-info btn-sm">详细</a>
                            <a href="{{route('admin.admin_user.edit', $item)}}" class="btn btn-bitbucket btn-sm">修改</a>
                            <button type="submit" form="delForm{{$item->id}}" class="btn btn-danger btn-sm" title="删除" onclick="return confirm('是否确定？')">删除</button>
                            <form class="form-inline hide" id="delForm{{$item->id}}" action="{{ route('admin.admin_user.destroy', $item) }}" method="post">
                                {{ csrf_field() }} {{ method_field('DELETE') }}
                            </form>
                            <a href="{{route('admin.admin_user.role', $item)}}" class="btn btn-default btn-sm">修改角色</a>
                            <a href="{{route('admin.admin_user.permission', $item)}}" class="btn btn-warning btn-sm">修改权限</a>
                        </td>
                    </tr>
                @endforeach
                </tbody>
